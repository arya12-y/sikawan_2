...
function Bidang() {
  ...
  const load = useCallback(async () => {
    setLoading(true)
    try {
      const res = await api.get('/bidangs', { params: search ? { search } : {} })
      setRows(normalize(res.data))
    } catch (e) { alert(e.response?.data?.message || 'Gagal memuat') } finally { setLoading(false) }
  }, [search])

  const loadRefs = useCallback(async () => {
    try { const res = await api.get('/opds'); setOpds(normalize(res.data)) } catch { setOpds([]) }
  }, [])

  useEffect(() => { queueMicrotask(() => { load(); loadRefs() }) }, [load, loadRefs])

  const openCreate = () => { setEditing(null); reset({ opd_id: '', nama: '', deskripsi: '', is_active: 1 }); setShowForm(true) }
  ...
  return (
    <div>
      {!showForm && <div className="card shadow-sm border-0">
        <div className="card-body">
          ...
          <div className="row g-2 mb-4">
            <div className="col-md-5"><div className="input-group"><span className="input-group-text bg-light border-end-0"><i className="bi bi-search"></i></span><input className="form-control border-start-0 ps-0" placeholder="Cari bidang..." value={search} onChange={(e) => setSearch(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && load()} /></div></div>
      ...
      {showForm && <div className="card shadow-sm border-0 mt-4"><div className="card-body p-4"><form onSubmit={handleSubmit(save)}>
        <div className="d-flex justify-content-between align-items-center mb-4"><h5 className="fw-bold mb-0">{editing ? 'Edit' : 'Tambah'} Bidang</h5><button type="button" className="btn btn-outline-secondary btn-sm" onClick={() => setShowForm(false)}><i className="bi bi-arrow-left me-1"></i>Kembali</button></div>
